...
init_per_suite(Config) ->
    ok = reltool_util:application_start(uuid, [], infinity),
    Config.

end_per_suite(_Config) ->
    ok = reltool_util:application_stop(uuid),
    ok.
...
t_wal_sequence0(Config) ->
    ...
    {TransId0, UUID1} = uuid:get_v1(UUID0),
    % request0
    ...
    {Chunk0, State1} = cloudi_write_ahead_logging:store_start(ChunkRequest0,
                                                              State0),
    {ChunkRequestId0, UUID2} = uuid:get_v1(UUID1),
    State2 = cloudi_write_ahead_logging:store_end(ChunkRequestId0,
                                                  Chunk0, State1),
    % request1
    {TransId1, UUID3} = uuid:get_v1(UUID2),
    ...
    {Chunk1, State3} = cloudi_write_ahead_logging:store_start(ChunkRequest1,
                                                              State2),
    {ChunkRequestId1, UUID4} = uuid:get_v1(UUID3),
    State4 = cloudi_write_ahead_logging:store_end(ChunkRequestId1,
                                                  Chunk1, State3),
    % request2
    {TransId2, UUID5} = uuid:get_v1(UUID4),
    ...
    {Chunk2, State5} = cloudi_write_ahead_logging:store_start(ChunkRequest2,
                                                              State4),
    {ChunkRequestId2, UUID6} = uuid:get_v1(UUID5),
    ...
    % request3
    {TransId3, UUID7} = uuid:get_v1(UUID6),
    ...
    {Chunk3, State8} = cloudi_write_ahead_logging:store_start(ChunkRequest3,
                                                              State7),
    {ChunkRequestId3, UUID8} = uuid:get_v1(UUID7),
    ...
    % request4 fails
    {TransId4, UUID9} = uuid:get_v1(UUID8),
    ...
    % request5
    {TransId5, UUID10} = uuid:get_v1(UUID9),
    ...
    {Chunk5, State13} = cloudi_write_ahead_logging:store_start(ChunkRequest5,
                                                               State12),
    {ChunkRequestId5, UUID11} = uuid:get_v1(UUID10),
    State14 = cloudi_write_ahead_logging:store_end(ChunkRequestId5,
                                                   Chunk5, State13),
    % request6
    {TransId6, UUID12} = uuid:get_v1(UUID11),
    ...
    {Chunk6, State15} = cloudi_write_ahead_logging:store_start(ChunkRequest6,
                                                               State14),
    {ChunkRequestId6, UUID13} = uuid:get_v1(UUID12),
    ...
    RetryF1 = fun({_QueueName, _Type, _Name, _Pattern, _RequestInfo, _Request,
        ...
        Age = (uuid:get_v1_time(erlang) -
               uuid:get_v1_time(TransId)) div 1000, % milliseconds
        if
            Age >= Timeout ->
                ?LOG_TRACE("new1: ~p (age ~p ms): timeout", [T, Age]),
                {error, timeout};
            true ->
                {NewTransId, _} = uuid:get_v1(UUID13),
    ...
    % request7
    {TransId7, UUID14} = uuid:get_v1(UUID13),
    ...
    {Chunk7, State19} = cloudi_write_ahead_logging:store_start(ChunkRequest7,
                                                               State18),
    {ChunkRequestId7, _} = uuid:get_v1(UUID14),
